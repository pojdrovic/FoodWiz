...
// This is the class which controls the search ingredients view controller!
import UIKit
...
class SearchViewController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    // This array stores the strings which are to be displayed on the tableView of possible ingredients.
    var autoComplete: [String] = []
    
    var ownedIngredients: [String] = []
    var selectedIngredients: [String] = []
    
    var selectedIngredientsMetaData: [Int] = []
    
    // Counter variable used for testing.
    var counter = 0
    
    // This array stores the ingredient metadata. When ingredients are added, their metadata tags will appear in this array
    var ownedIngredientsMetaData: [Int] = []
    
    
    // The NSUserDefaults information
    ...
    // This function is responsible for finding the elements in the dictionary for which 
    // the substring belongs in.
    // To increase reliability, the substring and the element in the dictionary are both made to be 
    // lowercased. However, only the original string is appended to the array of autocomplete strings.
    // This way, whether you are typing in uppercase or lowercase, the string results will remain the same.
    ...
    func searchAutocompleteEntriesWithSubstring(_ substring: String) {
        ...
        // This is the loop which interacts with the dictionary
        // Ingredient is the key (the string name of the ingredient)
        // and metaData is the value, or the int number of the ingredient which
        // allows us to effeiciently store which ingredients we have.
        for (ingredient, metaData) in dictOfIngredients{
            let searchString:NSString! = ingredient as NSString
            let lowercasedSearchString:NSString! = searchString.lowercased as NSString
        
            if(lowercasedSearchString.range(of: lowercaseSubstring).location != NSNotFound){
                autoComplete.append(ingredient)
            }
        }
        
        tableView.reloadData()
    } // end of search autocomplete possibilities function.
    ...
    // Function which deals with the small buttons next to each ingredient.
    @IBAction func logActions(sender: UIButton){
        print(sender.tag)
        let title = autoComplete[sender.tag]
        
        var arrayOfIngredients = UserDefaults.standard.object(forKey: "ingredients") as! [Int]
    
        if let value = dictOfIngredients[title]{
            print(value)
            print(type(of: value))
            // if the ingredient is already in the list of ingredients,
            // do nothing
            if (arrayOfIngredients.contains(value)){
                print("already found an element")
            }
            else {
                print("this is a new element")
                arrayOfIngredients.append(value)
    ...
    // Function which will add multiple ingredients to the list.
    @IBAction func buttonTapped(_ sender: Any) {
        print("addButtonTapped!")
        print(ownedIngredientsMetaData)
        var tempMetaDataArray: [Int] = []
        for key in selectedIngredients{
            tempMetaDataArray.append(dictOfIngredients[key]!)
        }
        
        var arrayOfIngredients = UserDefaults.standard.object(forKey: "ingredients") as! [Int]
        
        for element in tempMetaDataArray{
            if (arrayOfIngredients.contains(element)){
                print("doing nothing!")
            } else {
                print("new elements!")
                arrayOfIngredients.append(element)
            }
        }
        UserDefaults.standard.set(arrayOfIngredients, forKey: "ingredients")
        print(UserDefaults.standard.object(forKey: "ingredients") as! [Int])
